...
1️⃣ Detect objects using YOLOv5 (Ultralytics + PyTorch)
2️⃣ Continuously describe what’s in front of the camera
3️⃣ Speak the detected object names using TTS (pyttsx3)
...
Continuous speech every frame	Remove the (current_time - last_time > 3) condition (not recommended; too noisy)
Confidence threshold	Edit YOLOv5 detect file or add conf=0.4 in model call
...
Would you like me to extend this so it summarizes the entire scene using OpenAI (GPT-4o mini API) + TTS next — so it says things like “You are in a room with a chair and a laptop on a desk” instead of just naming objects?
